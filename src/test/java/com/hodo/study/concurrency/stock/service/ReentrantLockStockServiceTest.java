...
class ReentrantLockStockServiceTest {
	...
	@Autowired
	private ReentrantLockStockService reentrantLockStockService;
	...
	/**
	 * @Transactions과 synchronized를 같이 사용하는 경우 AOP 기반의 프록시 패턴으로 인해 동기화 제어 안됌
	...
	public void decreaseHundredTest () throws InterruptedException {
		...
		for (int i = 0; i < threadCount; i++) {
			executorService.submit(() -> {
				try {
					reentrantLockStockService.decrease(1L, 1);
				}finally {
					latch.countDown();
				}
